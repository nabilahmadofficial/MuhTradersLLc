<?php
// Breadcrumbs
if ( ! function_exists( 'sandbox_get_breadcrumbs' ) ) {
    function sandbox_get_breadcrumbs ( 
        $list_style = 'ul', 
        $list_id = 'breadcrumbs', 
        ...
        // Front page
        if ( is_front_page() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('Home', 'sandbox') . '</li>'; //get_bloginfo( 'name' )
        } else {
            $breadcrumb .= '<li><a href="' . home_url() . '">' . esc_html__('Home', 'sandbox') . '</a></li>'; //get_bloginfo( 'name' )
        }
        // Blog archive
        ...
        // Search
        if ( is_search() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( 'Search', 'sandbox' ) . '</li>';
        }
        // 404
        if ( is_404() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( '404', 'sandbox' ) . '</li>';
        }
        ...
        if ( is_single() && get_post_type() != 'post' && get_post_type() != 'attachment' ) {
            ...
            if ( get_post_type_archive_link( $cpt_obj->name ) ) {
                ...
                $archive_link = '';
                if ( 'ot_portfolio' == get_post_type() && sandbox_get_option('portfolio_archive') == 'archive_custom' ) {
                    $archive_link = get_page_link( sandbox_get_option('archive_page_custom') );                                
                } else {
...
if ( ! function_exists( 'sandbox_breadcrumbs' ) ) {
    function sandbox_breadcrumbs (){

        if( sandbox_get_option('breadcrumbs') && !is_front_page() ){ echo sandbox_get_breadcrumbs(); } ?>
